...
function App() {
  ...
  const setBoardColumns = (updater) => {
    ...
    setProjectBoards((prevBoards) => {
      ...
      const nextColumns = safeNextColumns;
...
const mergeSupabaseBoardIntoLocalState = (projectName, dbColumns = [], incomingDbTasks = []) => {
  ...
  const visibleBoardColumns = boardColumns.map((column) => ({
    ...
    tasks: (
      (column.tasks || [])
        .filter((task) => isTaskVisibleForProject(task, selectedProject))
        .filter((task) => !showOnlyMyTasks || isTaskAssignedToCurrentUserForCalendar(task))
    )
  }));
  ...
  const visibleArchivedTasks = archivedTasks
    .filter((task) => isTaskVisibleForProject(task, selectedProject))
    .filter((task) => !showOnlyMyTasks || isTaskAssignedToCurrentUserForCalendar(task));
...
const requirePermission = (permissionKey, message = 'Bu işlem için yetkin yok.') => {
  ...
  // Temizleme sadece bu hesabın Mesajlar panelindeki görünümü etkiler.
  // Gerçek proje mesajları ve diğer kullanıcıların mesajları silinmez.
  const clearedMessageIds = new Set(
    normalizeStorageArray(profilePreferences?.clearedMessageIds || [], [])
      .map((id) => String(id))
  );
  ...
  const messageItems = [
    ...
    .filter((message) => !clearedMessageIds.has(String(message.id)))
...
const filterTaskFollowersForSave = (people = []) =>
  ...
  const getHomeCalendarTasksForHour = (day, hour) =>
    getHomeTasksForCalendarDay(day).filter((task) => {
      const sourceDate =
        parseTaskDateValue(
          task.endDate ||
            task.dueDate ||
  ...
  const getHomeCalendarAllDayTasks = (day) =>
    getHomeTasksForCalendarDay(day).filter((task) => !formatMenuCalendarTaskTime(task));

  const homeCalendarListGroups = calendarWeekDays
    .map((day) => ({
      day,
      tasks: getHomeTasksForCalendarDay(day)
    }))
